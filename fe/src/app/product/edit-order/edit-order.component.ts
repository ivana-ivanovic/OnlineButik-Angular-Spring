...
export class EditOrderComponent implements OnInit {
  ...
  finishEditing(form: NgForm){
    ...
    if( this.selectedStatus === 'completed'){
      this.orderService.updateOrderWithStars(this.order, this.rating).subscribe(v => this.order = v.body!);
    } else {
      this.orderService.updateOrderWithoutStars(this.order).subscribe(v => this.order = v.body!);
    }
